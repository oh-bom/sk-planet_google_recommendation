...
def train_logistic_regression(train_df):
    ...
    # Logistic Regression 모델 생성 (고정된 하이퍼파라미터 사용)
    lr = LogisticRegression(labelCol="label", featuresCol="features", maxIter=100, regParam=0.1, elasticNetParam=0.8)
...
# 모델 학습 및 평가
model, feature_columns, val_auc, precision, recall, f1_score = train_logistic_regression(selected_df)
...
plt.ylabel('Feature')
plt.title('Feature Importances_logistic_regression')
